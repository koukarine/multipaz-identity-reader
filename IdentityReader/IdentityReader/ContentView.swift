    ...
    var body: some View {
            ...
            VStack {
                ...
                Button("Scan with NFC") {
                    Task {
                        ...
                        let reader = NfcTagReaderCompanion.shared.getReaders().first!
                        let scanOptions = NfcScanOptions(pollingFrameData: "6a0281030000".fromHexByteString())
                        let scanResult = try! await reader.scanMdocReader(
                            ...
                            //selectConnectionMethod: { connectionMethods in return connectionMethods.first() },
                            negotiatedHandoverConnectionMethods: connectionMethods,
                            nfcScanOptions: scanOptions,
        ...
        .onAppear {
            Task {
                ...
                // Try to prime and also ignore errors since this requires a network
                // connection and we might not have Internet access...
                //
                //let unusedKey = try? await appState.readerBackendClient.getKey(
